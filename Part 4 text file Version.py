    ...
    while True:
        ...
        if (option == 'y'):
            ...
            if cred_p and cred_d or cred_f in range_1:
                ...
                print('Out of range')
        if (option == 'q'):
            with open('report.text', 'w')as reportfile:
                for pro in Progress_L:
                    reportfile.write('Progress: ')
                    reportfile.write(str(pro))
                    reportfile.write('\n')
                for tra in Trailing_L:
                    reportfile.write('Trailing: ')
                    reportfile.write(str(tra))
                    reportfile.write('\n')
                for ret in Retriever_L:
                    reportfile.write('Retriever: ')
                    reportfile.write(str(ret))
                    reportfile.write('\n')
                for exc in Exclude_L:
                    reportfile.write('Exclude: ')
                    reportfile.write(str(exc))
                    reportfile.write('\n')
            with open('report.text', 'r')as reportfile:
                for read in reportfile:
                    print(read)
            break
except ValueError:
